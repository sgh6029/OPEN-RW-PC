/*
 * Decompiled with CFR 0.152.
 */
package com.corrodinggames.rts.game.units.a;

import com.corrodinggames.rts.game.units.a.ActionDisplayType;
import com.corrodinggames.rts.game.units.a.PopupQueueAction;
import com.corrodinggames.rts.game.units.BaseUnit;
import com.corrodinggames.rts.game.units.UnitType;
import com.corrodinggames.rts.game.units.custom.d.b;
import com.corrodinggames.rts.gameFramework.f.GameUIController;

public class QueueUnitAction
extends PopupQueueAction {
    UnitType a;

    public QueueUnitAction(UnitType as2) {
        this(as2, -999.0f);
    }

    public QueueUnitAction(UnitType as2, float f2) {
        super("u_" + as2.v());
        UnitType as3 = com.corrodinggames.rts.game.units.custom.l.c(as2);
        if (as3 != null) {
            as2 = as3;
            this.a("u_" + as2.v());
        }
        this.sortOrder = f2;
        this.a = as2;
    }

    @Override
    public String a() {
        String string2 = this.a.f();
        boolean bl2 = false;
        boolean bl3 = true;
        string2 = string2 + "\n\n" + com.corrodinggames.rts.gameFramework.f.GameUIController.a(BaseUnit.c(this.a), false, bl2, bl3);
        return string2;
    }

    @Override
    public String b() {
        return this.a.e();
    }

    @Override
    public int c() {
        return this.B().a();
    }

    @Override
    public b B() {
        b b2 = this.unitAction.a();
        if (b2 != null) {
            return b2;
        }
        return this.a.u();
    }

    @Override
    public b r_() {
        b b2 = this.unitAction.b();
        if (b2 != null) {
            return b2;
        }
        return this.a.B();
    }

    @Override
    public UnitType i() {
        return this.a;
    }

    @Override
    public float K() {
        return this.a.D();
    }

    @Override
    public ActionDisplayType f() {
        return ActionDisplayType.queueUnit;
    }

    @Override
    public boolean n_() {
        return !this.a.C();
    }

    @Override
    public boolean g(BaseUnit am2) {
        if (this.i().w()) {
            return true;
        }
        return super.g(am2);
    }

    @Override
    public boolean g() {
        return true;
    }
}

